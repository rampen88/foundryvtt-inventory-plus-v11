...
const API = {
  ...
  calculateWeightFromActorId(actorIdOrName) {
    const actorEntity =
      game.actors?.get(actorIdOrName) || game.actors?.getName(actorIdOrName);
    if (!actorEntity) {
  ...
  calculateWeightFromActor(actorEntity) {
    ...
    if (
      ...
      const encumbranceData = game.modules
        ...
        ?.api.calculateWeightOnActor(actorEntity);
    ...
    let totalWeight = inventoryItems.reduce((weight, item) => {
      ...
      let backpackManager = false; //retrieveBackPackManagerItem(item);
      if (backpackManager) {
        // Does the weight of the items in the container carry over to the actor?
        const weightless =
          getProperty(item, "system.capacity.weightless") ?? false;
        // const backpackManagerWeight =
        // 	API.calculateWeightOnActor(backpackManager)?.totalWeight ?? itemWeight;
        const backpackManagerWeight = calculateBackPackManagerWeight(
          item,
          backpackManager,
          ignoreCurrency
        );
        itemWeight = weightless
          ? itemWeight
          : itemWeight + backpackManagerWeight;
      ...
      const invPlusActiveTmp = true;
      if (invPlusActiveTmp) {
        // Retrieve flag 'categorys' from inventory plus module
        const inventoryPlusCategories = actorEntity.getFlag(
          CONSTANTS.MODULE_NAME,
          "categorys"
        );
      ...
      debug(
        `Actor '${actorEntity.name}', Item '${
          item.name
                 ...
                 ${itemQuantity} * ${itemWeight} = ${appliedWeight} on total ${weight} => ${
    ...
    const value =
      totalWeight && is_real_number(totalWeight)
        ? totalWeight.toNearest(0.1)
        : 0;
  ...
  isAcceptableType(categoryRef, itemData) {
    if (categoryRef.explicitTypes && categoryRef.explicitTypes.length > 0) {
      ...
      if (
        acceptableTypes &&
        acceptableTypes.length === 1 &&
        acceptableTypes[0]?.id === ""
      ) {
        return true;
      }
      let isOk = false;
  ...
  getItemsFromCategory(actor, categoryDatasetType, customCategorys) {
    return actor.items.filter((item) => {
      ...
      let sectionCategoryId = getProperty(
        item,
      ...
      // }
      if (
        sectionCategoryId === undefined ||
        customCategorys[sectionCategoryId] === undefined
      ) {
  ...
  async addCategory(
    ...
    if (items && items.length > 0) {
      for (const itmData of items) {
        ...
        if (!itemOnActor) {
          //@ts-ignore
          itemOnActor = await actorEntityTmp?.createEmbeddedDocuments("Item", [
            itmData,
          ]);
        }
        await itemOnActor.setFlag(
          CONSTANTS.MODULE_NAME,
