import { getApi, setApi } from "../module.js";
...
const module = {
  async manageInventoryPlus(targetActor, targetSheet, actorData) {
  ...
  async renderActorSheet5eCharacterInventoryPlus(...args) {
    ...
    const actorEntityTmp = game.actors?.get(actorData.actor._id);
    ...
    let inventoryPlusItemTypeCollection = [];
  ...
  async onDropItemInventoryPlus(wrapped, ...args) {
    ...
    // Check if the item is from a actor some reference uuid
      ...
      sourceActor = sourceActorId ? game.actors?.get(sourceActorId) : null;
    ...
    if (sourceActorId !== this.object.id || itemDataToCheck === undefined) {
      if (!targetActor.items.get(itemToCheck?.id)) {
    ...
    let dropedItem = this.object.items.get(itemToCheck?.id);
    if (!dropedItem) {
      if (createdItem) {
    ...
    const siblings = this.object.items.filter((i) => {
      const type = this.inventoryPlus.getItemType(i);
